...
public interface UserSubDisciplineRepository extends JpaRepository<UserSubDiscipline,Long> {
    ...
    match_scores AS (
        SELECT
            ...
            CASE
                WHEN usd1.level BETWEEN 1 AND 3 AND usd2.level BETWEEN 8 AND 10 THEN 15    --раздача очков, самая большая разнцица=15
        ...
        JOIN user_sub_discipline usd2
            ON usd1.sub_discipline_id = usd2.sub_discipline_id --рассмотр лишь общих дисциплин
    ...
    SELECT
        ...
        MAX(CASE WHEN direction = 'student_2_help' THEN sub_discipline_name END) AS student_2_help_subjects,  --взаимная помощь
        SUM(score) AS total_score --рассчет
    FROM match_scores
    WHERE score > 0
    GROUP BY student_1_id, student_2_id
    ORDER BY total_score DESC
""", nativeQuery = true)
    //ищет по фильтру, тоесть сам выбирает предметы
        ...
        WITH match_scores AS (
            SELECT
                ...
                CASE
                    ...
                    WHEN usd1.level BETWEEN 8 AND 10 AND usd2.level BETWEEN 6 AND 7 THEN 5  -- просмотр всех случаев как студенты могут быть друг другу полезны
                    ELSE 0
                END AS score,
                CASE
